...
// @version      1.1
// @description  强大的CV大师,集成到平台上,获取平台地址,使debug更方便
// @author       程序员战士
...
var ClickBtn = '<ul class="list">' +
    ...
    '<a id="openWorkbench" class="extBtn" style=""><i class="icon fa fa-coffee"></i>打开工作台</a>' +
...
(function () {
    ...
    $("#openWorkbench").click(function () {
        var regWidget = /Form\/EditForm\/(\S{36})|Form\/ValidForm\/(\S{36})\/edit\/(\S{36})/;
        var _url = window.location.href;
        var match = regWidget.exec(_url);
        if (match && match.length > 3) {
            _url = encodeURIComponent(_url);
            window.open("http://localhost:9900/workbench/" + _url);
        } else
            Power.ui.warning("url格式不支持导入");
    });
    ...
    if (typeof (formconfig) != "undefined") {
        if (formconfig == null)
            formconfig = {}
        config.formid.value = formconfig.FormId || FormId;
    ...
    var formid = FormId||getParameter("FormId");
    console.log("formId", formid);
    if (formid)
        config.formid.value = formid
    else {
